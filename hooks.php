...
add_hook('AdminInvoicesControlsOutput', 1, function($vars) {
	...
	<?php if ($creditId = invoice_is_credited($vars['invoiceid'])[1]): ?>
		...
		<a href="invoices.php?action=edit&id=<?= $creditId ?>" class="button btn btn-default">Voir la note de credit</a>

	<?php elseif ($originalId = invoice_is_creditnote($vars['invoiceid'])[1]): ?>

		<a href="invoices.php?action=edit&id=<?= $originalId ?>" class="button btn btn-default">Voir la facture original</a>
	...
	<?php else: ?>
		...
		<form method="POST" action="addonmodules.php?module=credit_invoice" name="credit_invoice_actions" style="display:inline;margin-top: 5px;">
			...
			data-toggle="tooltip"
			data-placement="left"
			data-original-title="Cliquez pour copier la facture sur une note de crédit.">Créer une note de crédit</button>
		</form>
